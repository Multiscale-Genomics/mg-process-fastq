...
class process_rnaseq:
    ...
    def run_kallisto_indexing(self, data_dir, species, assembly, e_release):
        ...
        cdna_file = data_dir + species + '_' + assembly + '_' + str(e_release) + '/' + species + '.' + assembly + '.cdna.all.fa.gz'
    ...
    def run_kallisto_quant(self, data_dir, species, assembly, e_release, project, fastq = [], single = False):
        """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--e_release", help="Ensembl Release (83)")
    ...
    try:
        os.makedirs(data_dir + species + "_" + assembly + "_" + str(e_release))
    except:
        pass
    ...
    # Get the cDNA
    cf.getcDNAFiles(data_dir, species, assembly, e_release)
    
    # Index the cDNA
    prs.run_kallisto_indexing(data_dir, species, assembly, e_release)
    
    # Quantify RNA-seq
    if len(in_files) == 1:
        prs.run_kallisto_quant(data_dir, species, assembly, e_release, project, in_files, True)
    else:
        prs.run_kallisto_quant(data_dir, species, assembly, e_release, project, in_files)
